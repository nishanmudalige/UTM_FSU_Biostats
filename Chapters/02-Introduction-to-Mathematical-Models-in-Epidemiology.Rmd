...
### Approach to Compartmental Modelling {#sec:compartments}

Referring back, compartmental modelling (*Definition* \@ref(def:Compartmental)) is the framework for prediction in epidemiology. The fundamental thinking behind the most common compartmental model, the SIR model, is that at any given point, a person will fall into either the Susceptible, Infected, or Recovered compartments, under the model's assumptions.
...
Mathematically, this is expressed in terms of the basic reproduction number, $R_0$ (*Definition* \@ref(def:R0)). If a proportion p of the population is immunized, the effective reproduction number becomes $R_0(1-p)$. We will define the Effective Reproduction Number in Chapter 3 (*Definition* \@ref(def:Effective)). To prevent the disease from becoming endemic (*Definition* \@ref(def:Endemic)), this effective reproduction number must be below one, leading to the inequality:
...
A population is said to have herd immunity (*Definition* \@ref(def:Herd)) if a great enough fraction of the population has been immunized so that the disease cannot become endemic (*Definition* \@ref(def:Endemic)). 
