...
- Fitting the SIQR Model
  ...
  - Using these parameters, and fitting the data, the model reveals, as expected, that the Infected compartment spikes before the quarantined compartment, accounting for spread of disease before transferring to the Quarantined compartment. Below is a plot showing these relationships.
...
We will use the NGM to solve an example of SAIR modelling COVID-19 while excluding vital dynamics (birth, death) to account for a short-term model. This example was modified from a paper by Ouyang et al. (2025) [@ouyang2025]. 
...
- $\frac{uI}{1+kI}$: saturated treatment function as the Holling Type 2 Functional Response **[cite: ]**
