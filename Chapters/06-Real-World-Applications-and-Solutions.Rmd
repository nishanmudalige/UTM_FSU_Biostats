...
articles cited. [@ramponi2024] [@zhu2024] 
The model takes into account vaccination
...
By plugging these parameter values into the previously solved equation for $R_{vac}$, we obtain $R_{vac} = 2.4716*10^4$ . Thus, we conclude that the endemic equilibrium is stable and the disease will persist. 

Parameter values used: b = 0.0085, d = 0.005, theta = 0.6, beta = 0.29, delta = 0.005, Lambda = 10, alpha = 0.0099, gamma = 0.2

**I am not really sure why the vaccination line is on the axis? (Check this!)**

```{r vsirplot, echo=FALSE, fig.align='center', fig.align='center', out.width='60%', fig.cap='Time series plot of VSIR model at endemic equilibrium, simulated with parameter values.'}
...
V0 <- Lambda*gamma/(d*(delta+d+gamma))
S0 <- Lambda*(delta+d)/(d*(delta+d+gamma))
N_dfe <- V0 + S0 

# Initial conditions: small outbreak with small A, I, and S slightly below DFE
svir_initial <- c(V = V0 * 0.99, S = S0 * 0.99, I = 0.01, R = 0)
