...
```{r vsir_diagram, echo=FALSE, engine='tikz', fig.ext='png', out.width='60%', fig.cap='The VSIR compartmental model.'}
...
ggplot(siqr_results, aes(x = time, y = Population, color = Compartment)) +
  ...
  labs(title = "Time Series Plot for SIQR Model",
       subtitle = paste0("N = ", N, ", α = ", siqr_param["alpha"], 
...
Write a closing thing for this
...
In this section we will use the NGM to solve an example of SAIR modelling COVID-19. Included in this model are vital dynamics (birth, death), and vaccination of individuals as a parameter rather than an additional compartment. This example was modified from a paper by Ouyang et al. (2025) [@ouyang2025].

This example showcases how models can become increasingly complex to solve and interpret when more parameters are involved. 
...
We use the following parameters to display real applications of data. 

We will analyze the $R_0$ value by fitting the model with the following parameters. We choose the parameter values based on examples in the article, and United States of America demographic data from the year 2020.  
let recruitment rate $\Lambda = 1$, transmission rate $\beta = 0.05$, $\alpha = 0.00025$, $\mu = 0.014$, $\delta = 0.04$, $\gamma = 0.12$, $p = 0.0065$, $\epsilon = 0.25$, COVID-19 death rate $theta = 0.01$, saturation function $u = 0.25$, $k = 20$. 

$$
R_0 = \frac{3.1828 × 10^{−7} \times 1 [(1-0.0065)0.00025(0.25+0.014+0.01)+(1-0.0065) 0.25+0.0065(0.12+ 0.25+0.014)]}{(0.014+0.04)(0.12+0.25+0.014)(0.25+0.014+0.01)} \\ 
R_0 = \frac{3.1828 × 10^{−7} [(0.9935)0.00025(0.274)+(0.9935)0.25+0.0065(0.384)]}
{(0.054)(0.384)(0.274)} \\ 
R_0 = \frac{3.1828 × 10^{−7} [(6.805475×10^{−5})+(0.248375)+(0.002496)]}
{0.005681664} \\
R_0 = \frac{(3.1828 × 10^{−7}) (0.2509391)} {0.005681664} \\
R_0 = 1.405731 × 10^{-5}
$$

Since $R_0 = 0.000014 < 1$, we determine that DFE is stable and the disease will die out. 

try again with beta = 0.05
$$
R_0 = \frac{3.1828 × 10^{−7} \times 1 [(1-0.0065)0.00025(0.25+0.014+0.01)+(1-0.0065) 0.25+0.0065(0.12+ 0.25+0.014)]}{(0.014+0.04)(0.12+0.25+0.014)(0.25+0.014+0.01)} \\ 
R_0 = \frac{3.1828 × 10^{−7} [(0.9935)0.00025(0.274)+(0.9935)0.25+0.0065(0.384)]}
{(0.054)(0.384)(0.274)} \\ 
R_0 = \frac{3.1828 × 10^{−7} [(6.805475×10^{−5})+(0.248375)+(0.002496)]}
{0.005681664} \\
R_0 = \frac{(0.05) (0.2509391)} {0.005681664} \\
R_0 = 2.208324
$$

Since $R_0 = 2.208 > 1$, we determine that EE is stable and the disease will persist. 
