...
In this chapter, we will solve a variety of four-compartment models of 
varying complexity: the SIQR, SVIR, and SAIR models. 
These models are derived from scholarly articles and experiments in which
the parameters, variables, and differential equations are provided. 
In this, we can apply our knowledge and techniques from previous chapters 
to analyze different compartmental models. After solving and analyzing these 
models, we will also look at some specific examples to understand the 
usage of these models with real data. 

## The SIQR Model

This model is based on the parameters and variables defined in the
article by Kalachev et al. (2023) and is used to model COVID-19. [@kalachev2022] 

The "Q" compartment represents quarantined individuals. 

It is important to note that the authors state their limitations, as
...
```{r siqrdiagram, echo=FALSE, fig.align='center', engine='tikz', out.width='60%', fig.ext='png', fig.cap='The SIQR compartmental model.'}
...
- Fitting the SIQR Model
  ...
  - Using these parameters, and fitting the data, the model reveals, as expected, that the Infected compartment spikes before the quarantined compartment, accounting for spread of disease before transferring to the Quarantined compartment. Figure \@ref(fig:siqrplot) is a time series plot showing these relationships. 
  
  - From these parameter values, we also obtain that $R_0 = 2.87 > 1$. Therefore, the endemic equilibrium is stable. 

```{r siqrplot, echo=FALSE, fig.align='center', out.width='70%', fig.cap='Time series plot of SIQR model at endemic equilibrium, simulated with parameter values.'}
...
The model in this example is based on the parameters and variables defined in the
articles by Ramponi et al. (2024) and Zhu X et al. (2024). [@ramponi2024] [@zhu2024] 

A non-specific disease that considers vaccination of the population is modelled. 
The model includes a "V" compartment for vaccinated individuals, and takes into account vaccination
rate, vaccine effectiveness, and vaccine induced decline in the immune
rate. 


```{r vsirdiagram, echo=FALSE, engine='tikz', fig.ext='png', out.width='70%', fig.cap='The VSIR compartmental model.'}
\usetikzlibrary{arrows.meta, positioning}
...
### Defining the Parameters and Model Formulation

With $N(t)$ representing the total population it can also be divided in
...
The following parameter values were retrieved from the article by Zhu et al. (2024) [@zhu2024].


- $b= 0.0085$ (cure rate of infected individuals )

- $d = 0.005$ (natural death rate)

- $\theta = 0.6$ (vaccine effectiveness rate)

- $\beta = 0.29$ (effective contact rate between susceptible and infected 
individuals)

- $\delta = 0.005$ (loss rate of immunity)

- $\lambda = 10$ (susceptible individuals recruited by immigration/birth)

- $\alpha = 0.0099$ (disease induced mortality)

- $\gamma = 0.2$ (vaccination rate of susceptible individuals)


By plugging these parameter values into the previously solved equation for $R_{vac}$, we obtain $R_{vac} = 2.4716*10^4 \gg 1$ . Thus, we conclude that the endemic equilibrium is stable and the disease will persist. 

Figure \@ref(fig:vsirplot) represents a simulation based on these parameter values that displays the populations in each compartment of the VSIR model over the period of 1 year. Due to the set-up of the model, the time series plot does not follow the usual trends of compartmental model time series plots. In Figure \@ref(fig:vsirplotzoom), we focus on the initial 10 days of the VSIR simulation. From this, we can see that the compartments change very drastically within a single day (this looks like a vertical drop in the Vaccination compartment of Figure \@ref(fig:vsirplot)). 

```{r vsirplot, echo=FALSE, fig.align='center', out.width='70%', fig.cap='Time series plot of VSIR model at endemic equilibrium, simulated with parameter values for 1 year time scale.'}
# Define the SVIR model function
svir_model <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    # Differential equations
    dV <- gamma*S - (1-theta)*beta*V*I - (gamma +d)*V
    dS <- Lambda + delta*V - beta*S*I - (gamma + d)*S
    dI <- beta*S*I + (1-theta)*beta*V*I - (d+ alpha + b)*I
    dR <- b*I - d*R

    list(c(dV, dS, dI, dR)) # Return the rates of change
  })
}

# Set parameters
svir_param <- c(b = 0.0085, d = 0.005, theta = 0.6, beta = 0.29, delta = 0.005, 
                Lambda = 10, alpha = 0.0099, gamma = 0.2) # at endemic equilibrium
# note that adjusting lambda changes the scale, while theta (vaccine effectiveness) doesn't change much
...
V0 <- round((Lambda*gamma)/(d*(delta+d+gamma)))
S0 <- round(Lambda*(delta+d)/(d*(delta+d+gamma)))
N_dfe <- V0 + S0 

# Initial conditions: small outbreak with small A, I, and S slightly below DFE
svir_initial <- c(V = V0 - 1, S = S0 - 1, I = 2, R = 0)

# Time points to solve for (0 to 1-years simulation (daily steps)
times <- seq(0, 365, by = 1)

# Solve the differential equations
svir_output <- ode(y = svir_initial, times = times, func = svir_model, parms = svir_param)

# Convert to data frame
svir_results <- as.data.frame(svir_output)

# Calculate derived quantities
svir_results <- svir_results %>% mutate(N = S + V + I + R, Day = time)

# PLOT TIME SERIES PLOT

plot_svir <- svir_results %>%
  pivot_longer(cols = c(V, S, I, R), 
               names_to = "Compartment", 
               values_to = "Count") %>%
  ggplot(aes(x = Day, y = Count, color = Compartment)) +
  geom_line(linewidth = 1.2, alpha = 0.8) +
  labs(subtitle = paste("β =", svir_param["beta"], ", α =", svir_param["alpha"],
                        ", Λ =", svir_param["Lambda"], 
                        ", δ = ", svir_param["delta"],
                        ", b = ", svir_param["b"], ", d = ", svir_param["d"], 
                        ", γ = ", svir_param["gamma"],
                        ", θ = ", svir_param["theta"]),
       x = "Time (days)", y = "Population (N)") +
  scale_color_manual(values = c("V" = "darkgoldenrod1",      # Vaccinated 
                                "S" = "cornflowerblue", # Susceptible
                                "I" = "tomato",   # Infected
                                "R" = "dodgerblue4")) +  # Recovered
  theme_minimal() +
  theme(legend.position = "bottom")
...
```{r vsirplotzoom, echo=FALSE, fig.align='center', out.width='70%', fig.cap='Time series plot of VSIR model at endemic equilibrium, simulated with parameter values for 10 days time scale.'}
# Time points to solve for (0 to 10 days simulation (daily steps)
times <- seq(0, 10, by = 1)
...
```{r sairdiagram, echo=FALSE, fig.align='center', engine='tikz', out.width='70%', fig.ext='png', fig.cap='The SAIR compartmental model.'}
...
```{r sairplot, echo=FALSE, fig.align='center', out.width='70%', fig.cap='Time series plot of SAIR model at endemic equilibrium, simulated with parameter values.'}
