...
In Chapter 3.4, we briefly introduced equilibrium and $R_0$ for examining stability in equilibrium. 

Chapter 4 will expand on the topic of equilibrium through **linear stability analysis**. This method provides the local stability of each equilibrium, as we approximate the solutions near the equilibrium. See Chapter 1.2 for the recommended linear algebra background. 
...
The stability of an equilibrium in a Compartmental Model is determined by the signage of the eigenvalues ($\lambda$) of the Jacobian Matrix (J). In this Chapter, we will solve for the eigenvalues of the SI and SIR model.
...
The Disease-Free Equilibrium (DFE) is the state in which the disease has been eradicated, resulting in the values for $I^*$ and $S^*$ below. It is important to note that this equation must retain the balance from our earlier equation ($S+I+R = 0$), so $R^* = O$.
...
Example:
$$\begin{bmatrix} 0 & 1 \\ 1 & 0\end{bmatrix}=I_2$$
...
The Endemic Equilibrium represents the state in which the Disease persists. Our equations for $S^*$ and $I^*$ are as follows. These will be integral to solving the Jacobian Matrix for the Endemic Equilibrium:
...
Using this general characterisitic equation, as seen when finding the eigenvalues of the Jacobian Matrix, we can understand how stable the equilibrium is in the system of differential equations. For stability to occur, all real roots and the real part of complex roots must be negative.
...
$$(+)(+)(+)$$
 ...
 - All terms are positive, denoted by each coefficient ($c_0, c_1, c_2,$) being positive.
   ...
   - Result: There are either 0 positive roots or 2 positive roots
