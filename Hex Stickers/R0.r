...
make_path <- function(id) {
  ...
  # End levels: mostly upward, a few downward (like your image)
  y_end <- sample(
    c(runif(1, 0.05, 0.25),  # down/flat
      runif(1, 0.25, 0.55),  # mild up
      runif(1, 0.55, 1.05)), # strong up
    size = 1,
  ...
  # Wiggle envelope: 0 at start, ramps up smoothly (keeps the start point crisp)
  env <- (1 - exp(-6 * x)) * (0.85 + 0.15 * (1 - x))
  
  # Multi-sine smooth wiggles (gives the “wavy” feel)
  m     <- sample(3:5, 1)
  ...
  # Add a bit of smooth random curvature (smoothed noise)
  nse <- smooth_noise(length(x), sd = runif(1, 0.03, 0.06), k = 51)
  nse[is.na(nse)] <- 0
  
  y <- baseline + env * (wiggle + 0.55 * nse)
  
  # HARD constraints: all start at same point; keep within view window
  y[1] <- y0
...
p <- ggplot(df, aes(x, y, group = id, colour = id)) +
  ...
  # blue start point (all curves share it)
  geom_point(
...
# ggsave("replicated_wavy.png", p, width = 7.5, height = 5.0, dpi = 300, bg = "white")
